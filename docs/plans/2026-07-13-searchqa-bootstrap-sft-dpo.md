...
2. Use 1,024 as the primary native-thinking cap. Caps 96 and 512 failed broadly; at 1,024, 31/32 outputs closed and one exact-cap outlier remained inside thinking. Retry only exact-cap malformed rows once at 2,048 after a per-row 4,096-token prompt-budget check. Log primary/retry token counts, original indices, and retry reason; fail on short malformed output or retry exhaustion.
