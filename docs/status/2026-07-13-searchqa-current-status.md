...
**Current staged recovery candidate:** `d651609fb3d77913aaafdbc381a90cac5dd480b1`
...
The original shard-2 recovery completed 994 trajectories. It contains 584 resolved trajectories, including 172 successful hinted student continuations and 37 strict student-only preference rows. The other three original jobs failed only at the response-preference invariant and were replaced by checkpoint-resumable jobs 14008, 14009, and 14010 from recovery commit `40391891343f7277073a1da45eec883b6f16f9ff`. At 20:37 IST all three had completed their primary 256-token teacher-thinking stages and were processing the explicit 1,024-token retry stage: shard 0 was at 128/270, shard 1 at 136/286, and shard 3 at 64/264, with no current OOM or traceback. Checkpoint selection and the DPO save/resume functional smoke are complete; final trajectory merge/audit, full preference construction, four-GPU optimization probing, reference log-probability computation, and reportable pilot DPO remain pending.

Full 1,000-example evaluations subsequently rejected checkpoints 150 and 200 as the DPO initialization. Checkpoint 150 reached `0.541` answer exact and `0.537` protocol exact; checkpoint 200 reached `0.538` and `0.535`. Both regress from checkpoint 50's `0.555` and `0.553`, so checkpoint 50 is frozen as the DPO start model. The 32-example scan was useful only for nominating these full evaluations and is not used as the final selection evidence.

The completed shard-2 preferences rebuild produced 37 unique student-only, no-hint rows at SHA-256 `919904db71e0d205a24fb9b6fa5ae8e326ba68520976d9272fea942c7f7ce57b`: 32 query pairs and 5 response pairs. Because direct training would overweight query behavior, commit `66d64ee` adds a deterministic fail-fast balanced DPO split. The node03 smoke split contains four query plus four response training rows and one query plus one response evaluation row, with zero example or row overlap. Exact train/eval SHA-256 values are `1645330604daf1bb1b39b97f25aca1d1724017387b92d33788831f4e4d74c791` and `db8db446e49fd805f7b5b9cfc9cb1549fe8b2e9e7e94e12a126d6623fda33beb`.

Checkpoint 50 and the balanced data were staged onto node03, and exact reference log probabilities were precomputed under the frozen prompt-context schema. Job 14046 failed before its first optimizer step because Arrow padded sparse nested `canonical_context` objects and changed the apparent data hash. Commit `8fc0b48` fixes the cause by hashing exact JSONL rows before Arrow conversion; no mismatched manifest was accepted. Retry 14047 then completed two real optimizer steps and wrote checkpoint 2, but Slurm killed its resume process at the 32 GB host-memory cgroup limit. Resume-only job 14048 requested 128 GB, resumed from checkpoint 2, completed steps 3 and 4, and verified checkpoint 4. This established real BF16 ZeRO-3 full-parameter DPO execution and save/resume correctness on two A100 GPUs; allocator cache-flush warnings confirm that the formal pilot must retain the four-GPU minimum.

A matched 32-example response audit compared frozen SFT checkpoint 50 with DPO checkpoint 4 under the same v2 prompts and deterministic decoding. The DPO smoke changed answer exact match from `0.46875` to `0.50000`, protocol exact match from `0.46875` to `0.50000`, malformed rate from `0.09375` to `0`, valid-format rate from `0.90625` to `1.0`, and retrieval recall@8 from `0.75000` to `0.81250`. All 32 DPO outputs were nonempty and cited. These directionally healthy results establish that the update did not collapse generation, but eight training rows and 32 audit rows are intentionally non-reportable; the merged balanced pilot and frozen 1,000-example validation remain required.
...
Downstream jobs 13954 (merge and trajectory audit), 13955 (preferences), 13956 (DPO split and reference log-probabilities), 13957 (optimization probe), 13958–13959 (four/eight-GPU scaling probes), 13989 (scale-decision freeze), and 13962 (DPO after the frozen gate) are submitted with explicit `afterok` dependencies and are currently pending. The pending DPO launcher job 13962 has been extended to a 24-hour wall limit so its synchronous smoke and full-run waits cannot be cut off by the former two-hour limit. DPO has not started.

At 14:11 IST a pre-execution audit found that the original pending freeze job 13960 referenced `/scratch/node10/aryama.murthy/...`, which does not exist, instead of the active lineage root `/scratch/node10/node10/aryama.murthy/...`. This was a queued launcher-path defect; no running collection or artifact was affected. Corrected job 13989 was submitted with `afterok:13959`, job 13962 was atomically repointed to `afterok:13989` while retaining its original exported DPO identities, and only then was job 13960 cancelled. `scontrol` and the generated batch script verify the repaired dependency and exact corrected path. No fallback or alias was introduced.

The resumable collector is staged separately on Turing at `~/.config/superpowers/worktrees/fixed-retrieval-v1/collector-resume-v2`, currently at exact commit `d651609fb3d77913aaafdbc381a90cac5dd480b1` on resumable base `b42dfa83ca824cd6554989aecbd6a0b96d90939a`. Local full verification passed 345 tests, 14 expected upstream warnings, and 171 subtests; focused compute-node verification of the latest candidate passed, alongside Ruff and shell syntax checks, and the earlier Turing orchestration subset passed 92 tests. The latest candidate adds per-call teacher-contract diagnostics, deterministic per-intervention teacher seeding, explicit manifest-bound teacher temperature/top-p/top-k controls, and a 24-hour wall limit on both the direct collector and the runtime-decision wrapper used to submit future resumable collector jobs. The wrapper mismatch was caught by a failing regression test before implementation; the same test passed locally and inside the node10 allocation after the one-line walltime correction. It does not retroactively alter the active producer lineage. Turing's login node cannot import the current PyTorch environment (`libtorch_cpu.so: failed to map segment from shared object`) and may abort even tiny `uv` environment creation under its constrained memory budget; therefore verification was run inside the existing node10 Slurm allocation with explicit numerical-library thread limits.
...
This staged worktree is a recovery path, not a launch-ready substitute for the active lineage. The running Python process and frozen generation optimization decision are both bound to commit `8562a4a9...`; the shared checkout was subsequently moved to `b8505d6b...`, while the already-loaded process continued from its launch identity. Resumability was introduced at base commit `b42dfa8...`; the current staged candidate is `d651609...` (the Turing cherry-pick of verified local commit `19540c2...`). A retry must first run and freeze a fresh measured generation decision for the exact recovery commit and decoding candidate, then launch into a new output lineage. Rewriting the old decision, mixing commits, or claiming that the current in-memory work can be resumed is forbidden.
