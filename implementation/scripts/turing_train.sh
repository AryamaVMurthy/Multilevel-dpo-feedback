...
export COMMIT_HASH CONFIG_HASH MODEL_HASH START_MODEL_SHA256 DATASET_HASH PROMPT_HASH RETRIEVAL_HASH SOURCE_SCHEMA_HASH PACKAGE_VERSIONS GPU_TELEMETRY ARTIFACT_PATHS MANIFEST_STARTED_AT RUN_MANIFEST OPTIMIZATION_DECISION OPTIMIZATION_DECISION_SHA256 SCALE_DECISION SCALE_DECISION_SHA256 CHECKPOINT_SMOKE_MANIFEST CHECKPOINT_SMOKE_MANIFEST_SHA256 DATASET_SOURCE DATASET_REVISION
...
ARGS+=(--model "$START_MODEL" --model-revision "$START_REVISION" --initial-checkpoint-sha256 "$START_MODEL_SHA256")
if [[ "$METHOD" == dpo ]]; then
